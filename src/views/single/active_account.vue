...
<template>
  <v-container>
    <v-row class="d-flex justify-center">
      <v-col md="5">
        <v-card :loading="loading ? 'blue' : null" :title="$t('app.name')" :subtitle="$t('user.active_account')">
          <v-card-text>
          ...
          <v-card-actions>
            <v-dialog v-model="dialog" width="500" persistent>
              <template v-slot:activator="{ props }">
                <v-btn variant="tonal"  v-bind="props" :text="$t('user.editNumber')" prepend-icon="mdi-account-edit"> </v-btn>
              </template>

              <template v-slot:default="{ isActive }">
                <v-card :loading="subloading ? 'danger' : null" :title="$t('user.editNumber')" prepend-icon="mdi-account-edit">
                  <v-form ref="formChangeNumber" :disabled="loading" fast-fail @submit.prevent="changeNumber()"  >
                    ...
                    <v-card-actions class="px-6">
                      <v-btn type="submit" :loading="subloading" variant="tonal" color="primary" :text="$t('user.editNumber')" prepend-icon="mdi-account-edit"> </v-btn>
                      <v-spacer></v-spacer>
                      <v-btn :disabled="subloading" @click="isActive.value = false" variant="tonal" color="secondary" :text="$t('dialog.cancel')" prepend-icon="mdi-close"></v-btn>
                    </v-card-actions>
                  </v-form>
                </v-card>
              </template>
