        ...
        <v-alert border="top" border-color="success" elevation="2">
          <v-container>
            <v-row>
              ...
              <v-col>
                <v-btn color="primary" title="ggg" to="/user/new-business" prepend-icon="mdi-archive-plus">{{
                  $t('title.user.business_create') }}</v-btn>
              </v-col>
            </v-row>
          </v-container>
        </v-alert>
    ...
    <v-row>
      <v-col>
        <v-expansion-panels on-update:model-value="setDefaults()" v-model="formData.bid">
          <v-expansion-panel v-for="(bus, index) in business" :value="bus">
            <template v-slot:title>
              <v-row>
                <v-col class="d-flex align-center">
                  <v-avatar class="me-2" :image="getApiUrl() + '/front/avatar/file/get/' + bus.id" size="40"></v-avatar>
                  {{ bus.name }}
                </v-col>
                <v-col class="">
                </v-col>
            ...
            <template v-slot:text>
              <v-row>
                <v-col cols="12" sm="12" md="4">
                  <v-select :items="bus.moneys" v-model="formData.money" item-title="label" item-value="props"
                    :label="$t('pages.dashboard.money')" return-object></v-select>
                </v-col>
                <v-col cols="12" sm="12" md="4">
                  <v-select :items="calendars" v-model="formData.calendar" item-title="name" item-value="props"
                    :label="$t('pages.create_business.calender')" return-object></v-select>
                </v-col cols="12" sm="12" md="4">
                <v-col>
                  <v-btn color="primary" prepend-icon="mdi-text" size="x-large" block :loading="loading"
                    @click="submit()">
